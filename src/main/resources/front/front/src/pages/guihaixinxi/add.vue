<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="物品状态" prop="wupinzhuangtai">
				<el-input v-model="ruleForm.wupinzhuangtai" 
					placeholder="物品状态" clearable :disabled=" false  ||ro.wupinzhuangtai"></el-input>
			</el-form-item>
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`guihaixinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
