	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/wupinxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">物品编号</div>
						<div class="text "  >{{detail.wupinbianhao}}</div>
					</div>
					<div class="item">
						<div class="lable">物品类别</div>
						<div class="text "  >{{detail.wupinleibie}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">租金</div>
						<div class="text "  >{{detail.zujin}}</div>
					</div>
					<div class="item">
						<div class="lable">租赁期限</div>
						<div class="text "  >{{detail.zulinqixian}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">租赁流程</div>
						<div class="text "  >{{detail.zulinliucheng}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('wupinxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('wupinxinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('wupinxinxi','租赁')" 
							@click="onAcross('zulindingdan','','','','')" type="warning">
							租赁
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="物品详情" name="first">
					<div class="ql-snow ql-editor" v-html="detail.wupinxiangqing"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/wupinxinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/wupinxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此物品信息？') .then(_ => {
					this.$http.post('wupinxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
