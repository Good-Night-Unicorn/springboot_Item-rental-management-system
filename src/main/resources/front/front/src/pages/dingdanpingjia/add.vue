		...
		<el-form
			...
			<el-form-item class="add-item" label="物品类别" prop="wupinleibie">
				<el-input v-model="ruleForm.wupinleibie" 
					placeholder="物品类别" clearable :disabled=" false  ||ro.wupinleibie"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item"  label="满意度" prop="manyidu">
			...
			<el-form-item class="add-item" label="评语" prop="pingyu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="评语"
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.manyiduOptions = "非常满意,满意,一般,不满意,其他".split(',')
			...
			// 多级联动参数
			info() {
				this.$http.get(`dingdanpingjia/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`dingdanpingjia/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
